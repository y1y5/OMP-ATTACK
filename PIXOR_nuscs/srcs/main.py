...
def train(exp_name, device):
    ...
    for epoch in range(st_epoch, max_epochs):
        ...
        progress_bar = tqdm(train_data_loader, desc=f"Epoch {epoch+1}", leave=False)

        for input, label_map, image_id in progress_bar:
            ...
            # 更新进度条的描述信息
            progress_bar.set_postfix({"Loss": loss.item(), "Cls Loss": cls, "Loc Loss": loc})
