...
class DoubleHeaderNumpyArray(object):
    ...
    def perturb_data(self, perturb_type: str):
        print(type(self.data))
        print(self.data.shape)
        print(self.header)
        x_lit = 1.0
        y_lit = 1.0
        r_lit = 1.0475 # 60 degrees range
        his_len = 4
        if perturb_type == 'position':
            for i in range(1, 1+his_len):
                # xd = random.uniform(-x_lit, x_lit)
                # yd = random.uniform(-y_lit, y_lit)
                # self.data[i, 0] += xd
                # self.data[i, 1] += yd
