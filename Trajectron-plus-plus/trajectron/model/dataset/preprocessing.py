...
def get_node_timestep_data(env, scene, t, node, state, pred_state,
    ...
    # Node
    timestep_range_x = np.array([t - max_ht, t])  # [-6, 2] x means inputs
    timestep_range_y = np.array([t + 1, t + max_ft])  # [3, 2] y means outputs
    ...
    rel_state[0:2] = np.array(x)[-1, 0:2] # curr frame state
...
def get_timesteps_data(env, scene, t, node_type, state, pred_state,
    ...
    nodes_per_ts = scene.present_nodes(t,
                                       ...
                                       return_robot=not hyperparams['incl_robot_node'])  # dict{timestep: [nodes]}
