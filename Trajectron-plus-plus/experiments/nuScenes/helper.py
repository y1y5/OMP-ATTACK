...
def load_model(model_dir, env, ts=3999, device='cpu'):
    ...
    model_registrar.load_models(ts) # 20
...
def plot_attacked_vehicle_mm(ax, predictions, dt, max_hl=10, ph=6, map=None, x_min=0, y_min=0, \
                    vis_mode='ego_adv', target_instance_id=None, other_vehicle=None):
    ...
    for node in node_list:
        ...
        if vis_mode == 'ego_adv':
            if node.id == 'ego':
                ...
                # plot predicted trajectories
                for sample_num in range(prediction_dict[node].shape[1]):
                    # print(predictions[:, sample_num, :, 0], predictions[:, sample_num, :, 1])
                    # print(predictions[:, sample_num, :, 0], predictions[:, sample_num, :, 1])
                    ax.plot(predictions[:, sample_num, :, 0], predictions[:, sample_num, :, 1], 'ko-',
                            zorder=750,
                            markersize=3,
                            linewidth=3, alpha=0.7) # 620

            elif other_vehicle is not None and node.id == other_vehicle:
                # plot vehicle
                r_img = rotate(cars[0], node.get(np.array([ts_key]), {'heading': ['°']})[0, 0] * 180 / np.pi, reshape=True)
                oi = OffsetImage(r_img, zoom=0.01, zorder=700)  # zoom: 0.025 -> 0.015
                veh_box = AnnotationBbox(oi, (history[-1, 0], history[-1, 1]), frameon=False)
                veh_box.zorder = 700
                ax.add_artist(veh_box)
