...
class FTOCP(object):
	...
	def buildandsolve(self,nodes,x0_val, xdes,ypreds,w):
			...
			for i in range(M):
				for j in range(Nnodes):
					for k in range(N):
						...
						# ### 2. new: Rectangular Buffers ###
						# # Calculate combined half-lengths and half-widths
						# half_length_sum = (self.L / 2 + nodes[j].length / 2)
						# half_width_sum = (self.W / 2 + nodes[j].width / 2)
						# # Apply constraints
						# constraint = vertcat(constraint, fabs(xbr[i][k,0] - ypreds[j][i][k,0]) - half_length_sum + slack[i,k])
						# constraint = vertcat(constraint, fabs(xbr[i][k,1] - ypreds[j][i][k,1]) - half_width_sum + slack[i,k])
						# ### 2. new: Rectangular Buffers ###
		...
		cost = 0
		cost_x   = 1.
		cost_y   = 5.  # 5. -> 1. to prevent vehicle close to roadside to stop , but didn't bring much improvement
		cost_acc = 5.0
		cost_ste = 20.0
		cost_slack = 1e6
